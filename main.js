 $(document).ready(function() {
 	...
 	navigator.geolocation.watchPosition(function(position) {
 		...
 		$('#degrees').css({
 			"visibility": "visible"
 					...
 					$("#degrees").text("This site requires geolocation permissions. To view, check your browser settings and reload the page.");
 	...
 	function getWeather(position) {
 		...
 		var ForecastIO = require(['forecast.io'], function(ForecastIO) {
 			...
 			forecast.getForecastToday(locations, function(conditions) {
				...
				var roundedTemp = Math.round(currentTemp);
 				setTempColor(roundedTemp);
 				setCurrentIcon(icon,roundedTemp);
 	...
 	$('input[id="slider"]').change(function() {
		...
		var sliderTemp = temperatures[$('#slider').val()];
 		setTempColor(sliderTemp);
 		setCurrentIcon(icons[$('#slider').val()], sliderTemp);
 	...
 	function setCurrentIcon(icon, temp) {
 		iconString = ""
 		if (temp > 70) {
 			iconString = icon + "-dark";
 		} else {
 			iconString = icon + "-light";
 		}
 	...
 	function setTempColor(currentTemp) {
		...
		isTempHigherThanNinety = (temperature > 90);
